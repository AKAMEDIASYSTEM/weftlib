...
Power up WEFT UI Board and try to guess the waveform of the texture
Press the encoder pushbutton to get an answer and change the mystery waveform

Manually, you can dig in and change the frequency (change the ff variable)
Are there some frequencies that are harder to judge frequency for?
...
// if knob turned, update freq
  if(newPosition!=oldPosition){
    oldPosition = newPosition;
    updateFreq(oldPosition);
    updateDisplay();
  }

// if button pressed, choose new rand waveform
  if (triggered) {
    triggeredV = false;
    // say "last waveform was: SQUARE" etc and randomly choose a new waveform
    if (DISPLAY_PRESENT) {
      updateDisplay();
    }
    wavelabel = random(0,4);
    switchTo(wavelabel);
  }
}
...
void updateFreq(int newFreq) {
  ff = constrain(floatmap(newFreq, encLoLimit, encUpLimit, minFreq, maxFreq), minFreq, maxFreq);
